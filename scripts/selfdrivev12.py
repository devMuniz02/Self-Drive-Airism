...
# -----------------------------------------------------------------
# DRAW LINES: ORIGINAL SEGMENTS (GREEN) + MERGED LINE (RED)
# -----------------------------------------------------------------
...
def draw_long_road_lines(base_image,
                         ...
                         roi_vertices=None):
    """
    - Canny sobre hsv_filtered_img (limitado por ROI si se da).
    - HoughLinesP.
    - Segmentos filtrados (longitud/ángulo) en VERDE.
    - Agrupa segmentos por ángulo cercano (angle_merge_eps_deg) y puntos próximos,
      ajusta una recta y dibuja UNA línea por grupo en ROJO.
    """
    ...
    # ⬇️ Aplicamos ROI también al mapa de bordes
    ...
    # --- Filtrar por longitud y ángulo, guardar segmentos ---
    ...
    for line in lines:
        ...
        # Distancia a horizontal (0 o 180)
    ...
    for (x1, y1, x2, y2, angle_deg) in segments:
        ...
        for g in groups:
            # compara ángulo con el promedio del grupo
            if abs(angle_deg - g["angle_mean"]) <= angle_merge_eps_deg:
                ...
                if min(d1, d2) <= endpoint_dist_thresh:
                    ...
                    g["angle_mean"] = (g["angle_mean"] * (len(g["points"]) - 2) + 2 * angle_deg) / len(g["points"])
                    assigned = True
                    break
    ...
    # --- Para cada grupo, una sola línea ROJA usando regresión lineal ---
    for g in groups:
        ...
        cv2.line(img_out, (x_min, y_min), (x_max, y_max), (0, 0, 255), 5)
...
# bottom row: MASK | CANNY(HSV FILTERED) | FOLLOW
# -----------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # Canny using HSV filtered image + ROI (para mostrar en grid)
            ...
            # Dibujar segmentos originales (verde) + líneas fusionadas (rojo), usando ROI en Hough
