...
# --- ROI Helper Functions (Unchanged) ---
def calculate_roi_points(height, width, top_y_p, top_x_p, top_w_p, bottom_y_p, bottom_x_p, bottom_w_p):
    ...
    vertices_for_warp = np.float32([
        ...
        (bottom_left_x, bottom_y), (bottom_right_x, bottom_y)
    ])
...
# -----------------------------------------------------------------
# --- 🤖 NEW: CANNY EDGE DETECTION ROAD FINDER 🤖 ---
# -----------------------------------------------------------------
# 
def process_image_canny_edges(img, roi_vertices, blur_kernel_size, canny_low, canny_high):
    """
    Finds lane lines using Canny edge detection.
    Returns a binary mask of the edges within the ROI.
    """
    ...
    # Canny finds edges between the low and high thresholds
    ...
    # Use bitwise_and to keep only edges within the ROI
    final_edges_mask = cv2.bitwise_and(edges, roi_mask)
    
    # Return the final mask for the tracker, and the raw Canny image for debug
    return final_edges_mask, edges


# --- 💡 MODIFIED: Blob Centering Function (FIXED STEERING) 💡 ---
def calculate_steering_from_blob(warped_mask):
    """
    Calculates the steering offset by finding the center
    of the road blob (or Canny edges) at a "look-ahead" point.
    """
    h, w = warped_mask.shape
    y_eval = int(h * 0.75) # Look-ahead line 75% down the image
    ...
    if len(indices) > 0:
        ...
        # --- 💡 FIX: Corrected offset logic ---
        # (Was car_center - lane_center, which is inverted)
        offset = lane_center - car_center
        
        return offset, (int(lane_center), y_eval)
    else:
        # Fallback: Try bottom half
        ...
        if len(indices) > 0:
            ...
            # --- 💡 FIX: Corrected offset logic in fallback ---
            offset = lane_center - car_center
...
# --- PID & Warp functions are UNCHANGED ---
...
def draw_center_visuals(original_img, warped_mask, center_point, matrix_inv):
    ...
    cv2.circle(color_warp, center_point, 10, (0, 0, 255), -1) # 10px radius, red, filled

    # Un-warp the red dot back to original perspective
...
def draw_steering_lines(image, steering_value):
    if steering_value is None: return image
    image_copy = image.copy(); h, w, _ = image_copy.shape
    start_left = (w // 2 - 40, h - 20); start_right = (w // 2 + 40, h - 20)
    line_length = h // 4; max_angle_deg = 30
    ...
    end_left_x = start_left[0] + end_x_offset; end_right_x = start_right[0] + end_x_offset
...
def create_dashboard(frontal_img, driver_img, reverse_img, follow_img, 
                     debug_image_1, debug_image_2, # Renamed for generic use
                     car_state, control_mode, steering_value=None):
    
    STD_HEIGHT = 200; STD_WIDTH = 250   
    ...
    try:
        ...
        # --- MODIFIED: Handle new debug images ---
        ...
        # Check if debug_2 is 1-channel (mask) and convert
    ...
    cv2.putText(front_resized, "FRONTAL (ROI & Target)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(debug_1_resized, "CANNY EDGES (RAW)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(debug_2_resized, "FINAL MASK (ROI)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    line_height = 20; padding = 15
    mode_text = f"MODE: {control_mode.upper()} (Press 'K' to toggle)"
    mode_color = (0, 255, 0) if control_mode == 'auto' else (0, 255, 255)
    cv2.putText(footer, mode_text, (padding, padding + line_height * 0), font, font_scale, mode_color, font_thickness)
    speed_mph = car_state.speed * 2.23694 
    speed_text = f"Speed: {speed_mph:.0f} MPH"
    cv2.putText(footer, speed_text, (padding, padding + line_height * 1), font, font_scale, (255, 255, 255), font_thickness)
    gear_text = f"Gear: {car_state.gear}"
    cv2.putText(footer, gear_text, (padding, padding + line_height * 2), font, font_scale, (255, 255, 255), font_thickness)
    if steering_value is not None:
        steer_text = f"Steer: {steering_value:.2f}"
        steer_color = (0, 0, 255)
        cv2.putText(footer, steer_text, (padding, padding + line_height * 3), font, font_scale, steer_color, font_thickness)
    if car_state.handbrake:
        ...
        cv2.putText(footer, hb_text, (hb_x, padding + line_height * 0), font, font_scale, (0, 0, 255), font_thickness)
...
if __name__ == "__main__":
    ...
    # Resize the CONTROLS window - we don't need it to be as tall
    cv2.resizeWindow(CONTROLS_WINDOW, 400, 350) 
    
    # --- ROI Position Trackbars (Unchanged) ---
    ...
    # --- NEW: Canny Edge Detection Trackbars ---
    # (We keep Blur from the old code)
    cv2.createTrackbar("Blur Kernel Size", CONTROLS_WINDOW, 5, 21, on_trackbar_change)
    cv2.createTrackbar("Canny Low Thresh", CONTROLS_WINDOW, 50, 255, on_trackbar_change)
    cv2.createTrackbar("Canny High Thresh", CONTROLS_WINDOW, 150, 255, on_trackbar_change)

    # --- DELETED: All RGB/HSV/Gray Trackbars ---
    
    # --- Control & PID Vars (Unchanged) ---
    car_controls = airsim.CarControls(); throttle = 0.0; steering = 0.0
    THROTTLE_INC = 0.1; STEERING_INC = 0.05; DECAY = 0.1 
    control_mode = args.mode; was_collided = False 
    BASE_THROTTLE = 0.35
    # --- Road Following PID Constants ---
    PID_P_GAIN = 0.0028; PID_I_GAIN = 0.0002; PID_D_GAIN = 0.0015
    pid_integral = 0.0; pid_previous_error = 0.0; last_timestamp = time.time()
    
    print(f"\nInitial control mode: {control_mode.upper()}")

    while True:
        try:
            # --- Collision & Key Handling (Unchanged) ---
            ...
            if is_collided_now and not was_collided:
                ...
                throttle = 0.0; steering = 0.0; car_controls.throttle = 0.0; car_controls.steering = 0.0
                client.setCarControls(car_controls); client.enableApiControl(True) 
                pid_integral = 0.0; pid_previous_error = 0.0
                print("WARNING: If car is seriously stuck, control may not be regained.")
            was_collided = is_collided_now
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'): break
            elif key == ord('k'):
                if control_mode == 'manual': control_mode = 'auto'
                else: control_mode = 'manual'
                pid_integral = 0.0; pid_previous_error = 0.0
                last_timestamp = time.time()
                print(f"Control mode set to: {control_mode.upper()}")
            elif key == ord('r'):
                print("--- ENVIRONMENT RESET ---"); client.reset(); client.enableApiControl(True)
                throttle = 0.0; steering = 0.0; car_controls.throttle = 0.0; car_controls.steering = 0.0
                car_controls.handbrake = False; pid_integral = 0.0; pid_previous_error = 0.0
                last_timestamp = time.time(); continue 

            # --- *** MODIFIED: Get SIMPLIFIED slider values *** ---
            ...
            # Get new Canny values
            blur_kernel_size = cv2.getTrackbarPos("Blur Kernel Size", CONTROLS_WINDOW)
            if blur_kernel_size % 2 == 0: blur_kernel_size += 1 # Must be odd
            if blur_kernel_size < 1: blur_kernel_size = 1
            canny_low = cv2.getTrackbarPos("Canny Low Thresh", CONTROLS_WINDOW)
            canny_high = cv2.getTrackbarPos("Canny High Thresh", CONTROLS_WINDOW)
            
            # --- DELETED: All RGB/HSV/Gray trackbar 'get' calls ---
            
            # --- Get Images & Run YOLO (Unchanged) ---
            ...
            # --- *** MODIFIED: Image Processing (NEW Canny Pipeline) *** ---
            ...
            # --- 💡 CALL THE NEW CANNY FUNCTION 💡 ---
            final_road_mask, canny_raw_debug = \
                process_image_canny_edges(raw_frontal, roi_poly_vertices,
                                          blur_kernel_size, canny_low, canny_high)

            # Prepare debug images for dashboard
            # Convert 1-channel Canny output to 3-channel BGR
            canny_debug_bgr = cv2.cvtColor(canny_raw_debug, cv2.COLOR_GRAY2BGR) 
            # final_road_mask is 1-channel, dashboard function will handle it
            ...
            if control_mode == 'manual':
                ...
                if key == ord('w'): throttle = min(1.0, throttle + THROTTLE_INC)
                elif key == ord('s'): throttle = max(-1.0, throttle - THROTTLE_INC)
                elif key == ord('a'): steering = max(-1.0, steering - STEERING_INC)
                elif key == ord('d'): steering = min(1.0, steering + STEERING_INC)
                elif key == ord(' '):
                ...
                car_controls.throttle = throttle; car_controls.steering = steering
                client.setCarControls(car_controls)
                current_steering_value = steering
            
            # --- AUTO MODE LOGIC (Unchanged) ---
            # This logic works perfectly with the new Canny mask!
            # 'calculate_steering_from_blob' will just find the
            # center of the Canny edge pixels.
            else:
                ...
                if center_point_viz is None:
                    auto_steering = 0.0; auto_throttle = BASE_THROTTLE
                    pid_integral = 0.0; pid_previous_error = 0.0
                    final_frontal_image = frontal_with_roi 
                else:
                    current_time = time.time(); delta_time = current_time - last_timestamp
                    last_timestamp = current_time; error = offset 
                    p_term = PID_P_GAIN * error
                    pid_integral += error * delta_time; pid_integral = np.clip(pid_integral, -500.0, 500.0) 
                    i_term = PID_I_GAIN * pid_integral
                    if delta_time > 0: derivative = (error - pid_previous_error) / delta_time
                    else: derivative = 0
                    d_term = PID_D_GAIN * derivative; pid_previous_error = error
                    ...
                    # --- Call Red Dot visualizer ---
                    final_frontal_image = draw_center_visuals(frontal_with_roi, warped_mask, center_point_viz, matrix_inv)

                car_controls.throttle = auto_throttle; car_controls.steering = auto_steering
                car_controls.handbrake = False
                client.setCarControls(car_controls)
                current_steering_value = auto_steering
            
            # --- Draw Steering Lines (Unchanged) ---
            if current_steering_value is not None:
                final_frontal_image = draw_steering_lines(final_frontal_image, current_steering_value)

            # --- *** MODIFIED: Dashboard Update *** ---
            car_state = client.getCarState()
            
            dashboard = create_dashboard(final_frontal_image, images['driver'], images['reverse'], 
                                         follow_cam_image, 
                                         canny_debug_bgr, # Pass new Canny debug
                                         final_road_mask, # Pass final edge mask
                                         car_state, control_mode, current_steering_value)
            
            if dashboard is not None:
                # Show in the main dashboard window
            ...
            if "Connection was closed" in str(e):
                ...
                try:
                    client.confirmConnection()
                    client.enableApiControl(True); print("Reconnected!")
                except Exception as recon_e:
                    print(f"Reconnect failed: {recon_e}. Exiting."); break
            else:
                if "polyfit" in str(e) or "concatenate" in str(e) or "nonzero" in str(e) or "mean of empty" in str(e):
                    print(f"CV Error (likely no road found): {e}")
                    if control_mode == 'auto':
                        pid_previous_error = 0.0
                        car_controls.steering = 0.0; car_controls.throttle = 0.1 
                        client.setCarControls(car_controls)
                else:
                    print(f"An error occurred in the loop: {e}"); break 

    # --- Cleanup (Unchanged) ---
    car_controls.throttle = 0.0; car_controls.steering = 0.0
