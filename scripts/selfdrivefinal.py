...
import warnings
# from numpy.lib.polynomial import RankWarning  # to suppress polyfit warnings

# Silence RankWarning from numpy.polyfit
...
# -----------------------------------------------------------------
# LANE BOUNDARY LOGIC (LINEAR FIT)
# -----------------------------------------------------------------

def get_x_at_y(coeffs, y, w):
    """
    Linear case: y = m x + b -> x = (y - b) / m
    coeffs = [m, b]
    """
    m, b = coeffs
    if abs(m) < 1e-6:
...
def limit_steering_by_lanes(auto_steering,
                            warped_mask_h,
                            warped_mask_w,
                            center_point_viz,
                            left_coeffs,
                            right_coeffs,
                            max_angle_deg=30.0,
                            max_lane_steer_limit=0.8):
    """
    If the blob center is outside [left_limit, right_limit] near the bottom,
    recompute steering as if the blob were at the INTERSECTION of the violated
    lane line with the TOP of the ROI (y=0 in warped coordinates).

    This keeps the car steering toward a point that is inside the lane and ahead.
    """
    if center_point_viz is None or left_coeffs is None or right_coeffs is None:
        return auto_steering
    
    cX, cY = center_point_viz

    y_bottom = warped_mask_h - 1
    y_top = 0  # top of warped ROI

    # Lane X at bottom (for lane width & limits)
    left_x_bottom = get_x_at_y(left_coeffs, y_bottom, warped_mask_w)
    right_x_bottom = get_x_at_y(right_coeffs, y_bottom, warped_mask_w)
    ...
    if left_x_bottom > right_x_bottom:
        left_x_bottom, right_x_bottom = right_x_bottom, left_x_bottom

    lane_width = right_x_bottom - left_x_bottom
    if lane_width <= 0:
        return auto_steering

    # Safe margin from lines (computed at bottom)
    margin = min(40, lane_width * 0.15)
    left_limit = left_x_bottom + margin
    right_limit = right_x_bottom - margin
    ...
    # Lane X at top (intersection with top of ROI)
    left_x_top = get_x_at_y(left_coeffs, y_top, warped_mask_w)
    right_x_top = get_x_at_y(right_coeffs, y_top, warped_mask_w)

    # If blob is beyond limits at bottom, project it to line-top intersection
    if cX > right_limit:
        side = "RIGHT"
        if right_x_top is not None:
            virtual_x = right_x_top
            virtual_y = y_top
        else:
            # fallback: clamp at bottom limit
            virtual_x = right_limit
            virtual_y = cY
        corrected = True
        ...
        if left_x_top is not None:
            virtual_x = left_x_top
            virtual_y = y_top
        else:
            # fallback: clamp at bottom limit
            virtual_x = left_limit
            virtual_y = cY
        corrected = True

    # If no violation, return original steering
    if not corrected:
        return auto_steering

    # Recompute steering using (virtual_x, virtual_y)
    car_center_x = warped_mask_w / 2.0
    dx = virtual_x - car_center_x
    dy = y_bottom - virtual_y
    if dy == 0:
        angle_rad = 0.0
    else:
        angle_rad = np.arctan2(dx, dy)
    ...
    max_angle_rad = np.deg2rad(max_angle_deg)
    new_steer = float(np.clip(angle_rad / max_angle_rad,
                              -max_lane_steer_limit,
                              max_lane_steer_limit))

    # PRINT each time the blob would cross the limit but is corrected
    print(f"[LANE-GUARD] Blob {side}-LIMIT correction to top-intersection: "
          f"orig_x={cX:.1f}, virt_x={virtual_x:.1f}, steer={new_steer:.3f}")

    return new_steer

# -----------------------------------------------------------------
# DRAW LINES: ORIGINAL SEGMENTS (GREEN) + LINEAR LANE LINES (RED)
# -----------------------------------------------------------------
...
def draw_long_road_lines(base_image,
                         ...
                         roi_vertices=None):
    """
    Identifies and draws lane lines with LINEAR fits (deg=1).
    Returns:
        img_out, left_lane_coeffs, right_lane_coeffs
        where coeffs are [m, b] for y = m x + b
    """

    if hsv_filtered_img is None or hsv_filtered_img.size == 0:
        return base_image, None, None
    ...
    # Apply ROI to the edges if exists
    ...
    groups = []  # each group: {"angle_mean", "points", "coeffs", "x_min", "x_max"}
    ...
    # Sort groups by mean x to define left/right
    sorted_groups = sorted(
        groups,
        key=lambda g: np.mean(np.array(g["points"])[:, 0]) if len(g["points"]) >= 2 else w / 2
    )
    ...
    for i, g in enumerate(sorted_groups):
        ...
        try:
            coeffs = np.polyfit(xs, ys, deg=1)  # LINEAR ONLY
            g["coeffs"] = coeffs
        except Exception:
            continue
...
def create_dashboard(frontal_img, driver_img, canny_img, follow_img, 
    ...
    try:
        ...
        driver_resized = front_resized = hsv_resized = mask_resized = canny_resized = follow_resized = blank_img
    ...
    cv2.putText(mask_resized, "FINAL ROAD MASK", (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(canny_resized, "CANNY (HSV+ROI)", (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(follow_resized, "FOLLOW (YOLO)", (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    
    combined_image_row1 = np.hstack([driver_resized, front_resized, hsv_resized])
    combined_image_row2 = np.hstack([mask_resized, canny_resized, follow_resized])
...
if __name__ == "__main__":
    ...
    cv2.createTrackbar("BlurKSize",     CTRL_WINDOW, 0,  21,  on_trackbar_change)
    ...
    cv2.createTrackbar("H_Low",   CTRL_WINDOW, 15,   179, on_trackbar_change)
    cv2.createTrackbar("H_High",  CTRL_WINDOW, 179, 179, on_trackbar_change)
    cv2.createTrackbar("S_Low",   CTRL_WINDOW, 51,   255, on_trackbar_change)
    cv2.createTrackbar("S_High",  CTRL_WINDOW, 248,  255, on_trackbar_change)
    ...
    cv2.createTrackbar("CannyLow",    CTRL_WINDOW, 250, 255, on_trackbar_change)
    cv2.createTrackbar("CannyHigh",   CTRL_WINDOW, 253, 255, on_trackbar_change)

    # Angle, min line length & angle merge epsilon
    cv2.createTrackbar("MinAngFromHor", CTRL_WINDOW, 10, 89, on_trackbar_change)
    cv2.createTrackbar("MinLineLen",    CTRL_WINDOW, 100, 500, on_trackbar_change)
    cv2.createTrackbar("MergeAngEps",   CTRL_WINDOW, 10, 45,  on_trackbar_change)
    ...
    while True:
        try:
            ...
            # Canny using HSV filtered image + ROI (for dashboard)
            hsv_gray = cv2.cvtColor(hsv_filtered_debug, cv2.COLOR_BGR2GRAY)
            ...
            if control_mode == 'manual':
                ...
                # Apply lane boundary limitation (use top intersection)
                if left_coeffs is not None and right_coeffs is not None and center_point_viz is not None:
                    auto_steering = limit_steering_by_lanes(
                        auto_steering,
                        warped_mask.shape[0],
                        warped_mask.shape[1],
                        center_point_viz,
                ...
                if center_point_viz is not None:
                    final_frontal_image = draw_center_visuals(
                        final_frontal_image, warped_mask, center_point_viz, matrix_inv
                    )
