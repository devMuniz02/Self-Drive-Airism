...
import time
import argparse
import warnings  # <-- ADDED for RankWarning
...
CAMERA_MAP = {
    ...
    "follow": "follow_cam" # <-- NEW "Eye View" Camera
}
TYPE_MAP = {
...
# --- MODIFIED: Accepts and displays 6 images total ---
def create_dashboard(frontal_img, driver_img, reverse_img, follow_img, frontal_blurry, frontal_canny, 
    ...
    try:
        ...
        follow_resized = cv2.resize(follow_img, (STD_WIDTH, STD_HEIGHT)) # <-- NEW
    ...
    cv2.putText(follow_resized, "FOLLOW", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2) # <-- NEW
    
    # Stack the 6 images in a 2x3 grid
    combined_image_row1 = np.hstack([driver_resized, front_resized, blurry_resized])
    combined_image_row2 = np.hstack([canny_resized, reverse_resized, follow_resized]) # <-- MODIFIED
...
if __name__ == "__main__":
    ...
    # --- Create Trackbars for ROI Position ---
