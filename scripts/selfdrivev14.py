...
def create_dashboard(frontal_img, driver_img, bird_img, follow_img, 
    ...
    cv2.putText(mask_resized, "FINAL ROAD MASK", (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(bird_resized, "BIRD-EYE (FRONT ROI)", (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # Canny using HSV filtered image + ROI (para debug, aunque ya no va al dashboard)
            ...
            # --- Bird-eye view (vista de águila) de la frontal usando la misma ROI ---
            try:
                bird_eye_view, _ = perspective_warp(raw_frontal, roi_warp_src)
            except Exception as e:
                print(f"[WARN] Bird-eye warp failed: {e}")
                bird_eye_view = raw_frontal.copy()
            ...
            # Dibujar segmentos originales (verde) + líneas polinomiales (rojo), usando ROI en Hough
            ...
            dashboard = create_dashboard(
                final_frontal_image,
                images['driver'],
                bird_eye_view,           # nueva vista de águila
                follow_cam_image, 
                hsv_filtered_debug,
