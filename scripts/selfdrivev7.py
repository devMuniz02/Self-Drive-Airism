...
def get_images(client):
    ...
    seg_req = airsim.ImageRequest(CAMERA_MAP["frontal"], TYPE_MAP["segmented"][0], TYPE_MAP["segmented"][1], TYPE_MAP["segmented"][2])
    ...
    try:
        ...
        cv2.putText(seg_img_bgr, "Segmented Error", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
def calculate_roi_points(height, width, top_y_p, top_x_p, top_w_p, bottom_y_p, bottom_x_p, bottom_w_p):
    ...
    top_y = height * (top_y_p / 100.0); top_x_center = width * (top_x_p / 100.0); top_width = width * (top_w_p / 100.0)
    bottom_y = height * (bottom_y_p / 100.0); bottom_x_center = width * (bottom_x_p / 100.0); bottom_width = width * (bottom_w_p / 100.0)
    top_left_x = top_x_center - (top_width / 2); top_right_x = top_x_center + (top_width / 2)
    bottom_left_x = bottom_x_center - (bottom_width / 2); bottom_right_x = bottom_x_center + (bottom_width / 2)
    vertices_for_poly = np.array([[(bottom_left_x, bottom_y), (top_left_x, top_y), (top_right_x, top_y), (bottom_right_x, bottom_y)]], dtype=np.int32)
    vertices_for_warp = np.float32([(top_left_x, top_y), (top_right_x, top_y), (bottom_left_x, bottom_y), (bottom_right_x, bottom_y)])
...
def create_dashboard(frontal_img, driver_img, reverse_img, follow_img, 
    ...
    cv2.putText(segmented_resized, "SEGMENTED (AirSim)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(road_mask_resized, "FINAL ROAD MASK", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
def create_color_preview_ui(h, w, 
    ...
    cv2.rectangle(ui_image, (10, 150), (w//2 - 10, 210), (int(bgr_low_color[0]), int(bgr_low_color[1]), int(bgr_low_color[2])), -1)
    ...
    cv2.rectangle(ui_image, (w//2 + 10, 150), (w - 10, 210), (int(bgr_high_color[0]), int(bgr_high_color[1]), int(bgr_high_color[2])), -1)
    ...
    cv2.putText(ui_image, str(gray_low_thresh), (low_pos - 10, 350), font, 0.5, (0, 255, 0), 1)
    
    cv2.line(ui_image, (high_pos, 250), (high_pos, 330), (0, 0, 255), 2) # High = Red
    cv2.putText(ui_image, str(gray_high_thresh), (high_pos - 10, 370), font, 0.5, (0, 0, 255), 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=['manual', 'auto'], default='manual', help="Initial control mode (default: manual)")
    parser.add_argument('--use_yolo', action='store_true', help="Enable YOLOv8 object detection (default: False)")
    ...
    try:
        ...
        print(f"Error connecting to AirSim: {e}"); exit()
    ...
    cv2.createTrackbar("Top Width %", CONTROLS_WINDOW, 50, 100, on_trackbar_change)
    ...
    while True:
        try:
            ...
            if key == ord('q'): break
                ...
                print("--- ENVIRONMENT RESET ---"); client.reset()
            ...
            final_road_mask, hsv_filtered_debug, road_mask_debug_for_dash = \
                process_image_3_stage_filter(raw_frontal, roi_poly_vertices, 
                                             blur_kernel_size, 
                                             rgb_r_low, rgb_r_high, rgb_g_low, rgb_g_high, rgb_b_low, rgb_b_high,
                                             hsv_h_low, hsv_h_high, hsv_s_low, hsv_s_high, hsv_v_low, hsv_v_high,
                                             gray_low, gray_high)
            ...
            # --- Manual Mode ---
            if control_mode == 'manual':
                # We don't need to do anything, user is driving
                # But we can get the current steering to display it
                car_state = client.getCarState()
                current_steering_value = car_state.kinematics_estimated.angular_velocity.z_val # Just an estimate
            ...
            dashboard = create_dashboard(final_frontal_image, images['driver'], images['reverse'], 
                                         follow_cam_image, 
                                         images["segmented"], road_mask_debug_for_dash,
                                         car_state, control_mode, current_steering_value)
            
            if dashboard is not None:
                cv2.imshow(DASHBOARD_WINDOW, dashboard)
            
            # --- *** NEW: Show Color Preview Window *** ---
            preview_ui = create_color_preview_ui(400, 300, # Height and Width of this window
                                                 rgb_r_low, rgb_r_high, rgb_g_low, rgb_g_high, rgb_b_low, rgb_b_high,
                                                 hsv_h_low, hsv_h_high, hsv_s_low, hsv_s_high, hsv_v_low, hsv_v_high,
                                                 gray_low, gray_high)
            ...
            if "Connection was closed" in str(e):
                ...
                try:
                    client.confirmConnection()
                    if control_mode == 'auto': client.enableApiControl(True)
                    print("Reconnected!")
