...
# -----------------------------------------------------------------
# --- 🤖 BLOB STEERING & VISUALIZATION (REPLACE THIS FUNCTION) 🤖 ---
# -----------------------------------------------------------------

def calculate_steering_from_blob(warped_mask):
    """
    Calculates steering offset AND the visual centroid of the road blob.
    
    Steering is based on a fixed look-ahead row (y_eval).
    Visualization is based on the centroid (center of mass) of the entire blob.
    """
    h, w = warped_mask.shape
    
    # --- 1. Steering Calculation (Stable Logic) ---
    # We use a fixed "look-ahead" row for stable steering
    y_eval = int(h * 0.75) 
    look_ahead_row = warped_mask[y_eval, :]
    
    indices_steer = look_ahead_row.nonzero()[0]
    offset = 0.0
    
    if len(indices_steer) > 0:
        lane_center = np.mean(indices_steer)
        car_center = w / 2
        offset = car_center - lane_center
    else:
        # Fallback: check histogram of bottom half if look-ahead row is empty
        histogram = np.sum(warped_mask[h//2:, :], axis=0)
        indices_hist = histogram.nonzero()[0]
        if len(indices_hist) > 0:
            lane_center = np.mean(indices_hist)
            car_center = w / 2
            offset = car_center - lane_center
        # If both fail, offset remains 0.0

    # --- 2. Visualization Centroid Calculation (NEW LOGIC) ---
    # This finds the "center of mass" (centroid) of the *entire* road blob.
    # This is what allows the 'Y' coordinate of the red dot to change.
    viz_point = None
    try:
        M = cv2.moments(warped_mask)
        if M["m00"] > 0: # Check if area (m00) is non-zero
            # Calculate the X and Y of the centroid
            viz_cX = int(M["m10"] / M["m00"])
            viz_cY = int(M["m01"] / M["m00"]) # <--- HERE is the new Y
    ...
    # --- 3. Return Values ---
    # offset: Used by the PID controller to steer the car
    # viz_point: Used by draw_center_visuals to draw the red dot (now has a dynamic Y)
    return 0.0, viz_point
...
def create_dashboard(frontal_img, driver_img, reverse_img, follow_img, 
    ...
    cv2.putText(hsv_filtered_resized, "HSV FILTERED", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
